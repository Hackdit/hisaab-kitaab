...
async function loadRedisState(phone: string): Promise<any> {
  try {
...
async function saveRedisState(phone: string, state: any): Promise<void> {
  try {
...
export function initializeInvoiceState(): InvoiceFlowState {
...
export async function handleInvoiceFlow(
  ...
  const invoiceState = state.invoiceFlow as InvoiceFlowState;
  ...
  function mapItems(nlpItems: any[]): InvoiceFlowState['items'] {
    return (nlpItems || []).map((item: any) => ({
  ...
  switch (invoiceState.step) {
    case 'start': {
      ...
      if (customerName) invoiceState.customerName = customerName;
      if (entities.gstin) invoiceState.customerGstin = entities.gstin;
    ...
    case 'payment_mode': {
      ...
      const validModes: Record<string, string> = { upi: 'UPI', cash: 'Cash', cheque: 'Cheque' };
      ...
      try {
        ...
        // ── 3. Calculate GST ──
        const { subtotal, cgst, sgst, igst, totalTax, grandTotal, roundOff, isInterstate } =
          calculateGST(gstItems, sellerState, buyerState);
        ...
        try {
          ...
          if (!buckets?.find((b: any) => b.id === bucketName)) {
        ...
        const ownerMsg =
          `✅ Invoice #${invoiceNumber} ban gaya!\n` +
        ...
        if (existingPhone && pdfUrl) {
          const customerMsg =
            `Namaste! 🙏 Aapka invoice aa gaya hai:\n\n` +
  ...
  if ((invoiceState as any).step !== 'start') {
...
function parseItemsFromMessage(message: string): Array<{ name: string; qty: number; unit: string; rate: number }> | null {
  const text = message.trim();
  if (!text) return null;

  // ── Step 1: Extract rate ──
  const RATE_PATTERNS: [RegExp, number][] = [
  ...
  let rateMatch: RegExpExecArray | null = null;
...
function generateConfirmationMessage(state: InvoiceFlowState): string {
