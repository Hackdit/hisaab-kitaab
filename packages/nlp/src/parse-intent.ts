// ──────────────────────────────────────────
// Hisab-Kitaab Intent Parser
// Uses Claude to extract structured business
// intent from Hindi/English/Hinglish messages.
// ──────────────────────────────────────────

import Anthropic from '@anthropic-ai/sdk';
import type { ParseInput, ParseOutput, IntentType, Entities } from './types.js';
...
/**
 * Parse user text into a structured business intent.
 * Uses Claude to understand Hindi/Hinglish and extract entities.
 *
 * Falls back to a local regex-based classifier if Claude is unavailable.
 */
export async function parseIntent(input: ParseInput): Promise<ParseOutput> {
  const { text, conversationState, businessContext } = input;

  // Build message with business context so Claude personalizes the response
...
function buildContextMessage(text: string, ctx: ParseInput['businessContext'], state: Record<string, unknown>): string {
...
function parseClaudeResponse(
  ...
  const entities: Entities = {
    ...
    items: Array.isArray(rawEntities.items) ? rawEntities.items as Entities['items'] : undefined,
  ...
  if (rawEntities.item_name) entities.item_name = String(rawEntities.item_name);
  if (rawEntities.quantity != null) entities.quantity = Number(rawEntities.quantity);
  if (rawEntities.unit) entities.unit = String(rawEntities.unit);

  // Pass through any extra entity fields
  for (const [key, val] of Object.entries(rawEntities)) {
    if (!(key in entities)) {
      (entities as Record<string, unknown>)[key] = val;
    }
  }
  ...
  // If confidence too low, override to unknown
  const finalIntent = confidence < 0.6 ? 'unknown' : intent;

  // If unknown intent, generate helpful reply
...
// ─── Local fallback (when Claude API is down) ────────────────────────

function localFallback(text: string): ParseOutput {
  ...
  // Check bank SMS first before generic payment
  ...
  // Subscription before udhaar (which matches "lena hai")
...
// ─── Utility: simple language guess (hi vs en) ──────────────────────

function guessLanguage(text: string): 'hi' | 'en' {
