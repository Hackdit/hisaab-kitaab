...
const HSN_MAP = new Map<string, HSNEntry>(HSN_TABLE.map(e => [e.hsnCode, e]))
 ...
 */
export function lookupHSN(hsnCode: string): HSNEntry | undefined {
 ...
 */
export function searchHSN(keyword: string, limit: number = 5): HSNEntry[] {
  const lower = keyword.toLowerCase()
  const direct = HSN_TABLE.filter(
    e => e.hsnCode === keyword || e.description.toLowerCase().includes(lower)
  )
  // Boost exact HSN matches, then sort by relevance
  direct.sort((a, b) => {
    if (a.hsnCode === keyword) return -1
    if (b.hsnCode === keyword) return 1
 ...
 */
export function isExemptGood(hsnCode: string): boolean {
