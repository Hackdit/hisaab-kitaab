 ...
 */
function calculateLineItemTax(item, index, isInterstate) {
...
function amountInWords(amount) {
    ...
    function convertIndian(n) {
        ...
        let result = '';
        if (crores > 0)
            result += convertBelow1000(crores).trim() + ' Crore ';
        if (lakhs > 0)
            result += convertBelow1000(lakhs).trim() + ' Lakh ';
        if (thousands > 0)
            result += convertBelow1000(thousands).trim() + ' Thousand ';
        if (hundreds > 0)
